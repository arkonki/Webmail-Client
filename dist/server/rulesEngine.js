import { SystemLabel, SystemFolder } from '../src/types.js';
import * as dbService from './databaseService.js';
export const checkCondition = (email, condition) => {
...
export const applyRulesToNewEmail = async (email, client, uid, userId) => {
...
export const getRuleActionsForEmail = async (email, userId) => {
    const settings = await dbService.getAppSettings(userId);
    if (!settings.rules || settings.rules.length === 0) {
        return { destinationFolderId: SystemFolder.SENT, flagsToAdd: [] };
    }
    let destinationFolderId = SystemFolder.SENT;
    const flagsToAdd = [];
    let hasMoveRuleApplied = false;
    for (const rule of settings.rules) {
