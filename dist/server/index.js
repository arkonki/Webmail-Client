...
import { SystemLabel } from '../src/types';
...
// Mail Actions
app.post('/api/conversations/move', async (req, res) => {
...
app.post('/api/conversations/delete-permanently', async (req, res) => {
...
app.post('/api/conversations/set-label-state', async (req, res) => {
...
app.delete('/api/emails/draft', async (req, res) => {
...
const startServer = async () => {
    ...
    server.on('upgrade', async (request, socket, head) => {
        ...
        try {
            ...
            wss.handleUpgrade(request, socket, head, (ws) => {
                ws.userId = session.userId; // Attach userId to the ws instance
    ...
    wss.on('connection', async (ws, req) => {
        ...
        const session = await dbService.getSession(url.parse(req.url, true).query.token);
        if (session) {
            try {
                const decryptedPassword = security.decrypt(session.encryptedCredentials);
